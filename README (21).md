...
│
├── room-business-lounge.html      # Room pages (14 total) — each is a self-contained
...
├── founders-toolkit.html           # Free "Founder's Toolkit" landing page — teases the 20+ document
│                                   #   kit (plan/run/grow/legal templates) and collects requests via
│                                   #   mailto; linked from virtual-glowbag.html's resource cards
├── harbor-emergency-housing.html   # Standalone emergency-housing directory ("Harbor"). NOT YET
│                                   #   linked from anywhere in the site — same concept as the "🏠 Harbor"
│                                   #   tab already built inside gccc-crisis-hotline.html. Integration
│                                   #   plan still pending as of this writing.
│
...
- All 14 room pages share the same header/footer and load
...
- **Back office:** `index.html`'s fixed header has a slim utility bar
  ...
  **Dashboard** (`back-office-dashboard.html`). This lives in
  `#site-header-wrap` above `#site-nav`. The sign-in page checks the
  entered staff ID and access code against a demo credential pair
  (`admin@gccc.org` / `GCCCAdmin2026!`) entirely in the browser, then redirects to
  `back-office-dashboard.html` on success; its "← Back to main site"
  link returns to `index.html`. From the dashboard, the **"← Back to
  Home"** link in the header returns to `index.html`, and clicking the
  user avatar signs out back to `back-office-sign-in.html`. The
  dashboard's sidebar switches between its Dashboard, Schedule, Staff
...
- **Added: "Global Citizens Magazine & Press" card on `virtual-glowbag.html`.**
  New resource card in the adult, kids, and pet item lists (right after
  the Project Gutenberg card) plus the "More Free Resources" summary
  grid, linking to `room-magazine.html`. Matching prize slice
  ("Magazine & Press") added to `gccc_prize_wheel.html` so the wheel
  stays in sync with the GlowBag's item list.
...
- **Added: `global-coin-exchange-demo.html`.** New standalone visual
  ...
  involved.
- **Added: `founders-toolkit.html`.** New standalone "Free Download"
  landing page for the Founder's Toolkit — a 20+ document kit for
  starting, running, and growing a business or organization. Linked from
  a "Founder's Toolkit" resource card on `virtual-glowbag.html`.
- **Fixed: incorrect room-page count in `README.md`.** Corrected "13
  total" / "All 13 room pages" to 14 in both places — 14 room pages are
  actually listed and shipped.
- **Fixed: `index.html` was missing its Back Office utility bar.** The
  README had long described a "🔒 Back Office" dropdown above the main
  nav, but `index.html` never actually had it — `back-office-sign-in.html`
  and `back-office-dashboard.html` were unreachable from the live site.
  Added `#site-header-wrap` + `#utility-bar` with the dropdown (Staff
  Sign In / Dashboard links) above `#site-nav`, and bumped `#hero`'s
  top padding from 80px to 112px to clear the taller header.
- **Fixed: dead "← Back to main site" link on `back-office-sign-in.html`.**
  It pointed to `href="#" onclick="return false;"` — now points to
  `index.html`.
- **Fixed: `back-office-sign-in.html` and `back-office-dashboard.html`
  each contained three duplicate copies of the entire document**
  concatenated together (three `<!DOCTYPE>`…`</html>` blocks stacked
  in one file — almost certainly a save/export accident). Deduplicated
  both down to a single clean, well-formed document each; no content
  was changed beyond removing the two redundant copies.
- **Not yet done:** `harbor-emergency-housing.html` (a standalone
  emergency-housing directory) was added to the repo but isn't linked
  from anywhere yet, and duplicates the "🏠 Harbor" tab already built
  into `gccc-crisis-hotline.html`. Integration plan still pending.
- **Fixed (again): white background on `index.html`.** The `:root{}`
  variable block and base `body{}` rule — added in an earlier fix — were
  missing again in this version of the file (likely lost when the file
  was re-exported/re-merged). Every dark section (`--navy`, `--navy2`,
  `--slate`, etc.) was rendering as browser-default white because all
  17 custom properties the stylesheet depends on were undefined. Re-added
  the full `:root{}` block plus `*{box-sizing:border-box}`,
  `html,body{margin:0;padding:0}`, and `body{background:var(--navy);...}`
  at the very top of the `<style>` block. If this recurs, check that
  future saves/exports of `index.html` keep this block intact — it's
  easy to lose when copy-pasting sections between versions.
